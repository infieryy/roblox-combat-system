...
The source of truth for existing work is branch **`cursor/master-firstperson-pickup-consensus-dd25`** (commit `9767000`). `main` is empty except the README; the real system lives in that branch as **two self-contained scripts**:
...
2. **Source material:** porting agents (1, 6, 8) pull logic from branch `cursor/master-firstperson-pickup-consensus-dd25`. Do not invent new physics — move the proven code.
